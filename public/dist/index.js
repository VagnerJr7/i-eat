document.addEventListener("DOMContentLoaded", function () {
    ...
    const productQuantityInput = document.getElementById("product-qtd-input");
    ...
    const renderProductList = () => {
        ...
        cartProducts.forEach((product) => {
            ...
            productItem.textContent = `${product.title} - Valor: R$ ${product.value.toFixed(2)} - Quantidade: ${product.quantity}`;
    ...
    if (carrinhoProdutosArray.length > 0 &&
        criarContaContainer &&
        modalTitleElement &&
        ...
        carrinhoProdutosArray.forEach(function (carrinhoProduto) {
            carrinhoProduto.addEventListener("click", function () {
                // Obtém o título e a URL da imagem do item clicado (ajuste conforme necessário)
                const productTitle = carrinhoProduto.querySelector(".title-product").textContent;
                const productValue = parseFloat(carrinhoProduto
    ...
    productQuantityInput.addEventListener("input", function () {
        ...
        const productPrice = parseFloat(modalValueElement.textContent.replace("Valor: ", "").trim());
    ...
    adicionarButton.addEventListener("click", function () {
        ...
        const productValue = parseFloat(modalValueElement.textContent.replace("Valor: ", "").trim());
